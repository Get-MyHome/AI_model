...
2026-09-02 전수감사 기준 extractor 0.2 `REVIEWED` 검수본은 0건이므로, 사람이 원본 PDF를
재검증해 승인하기 전에는 운영 판정에 쓰면 안 됩니다.
...
`tests/test_funding_stress.py`가 위 값과 정액 중도금·HALF_UP·경로 min/max 비합산·
미검수 거부를 회귀 검증합니다.
