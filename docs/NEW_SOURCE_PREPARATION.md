...
매일 오전 9시(Asia/Seoul)에 검사한다. 서버가 꺼져 예약을 놓쳤다면 다시 켰을 때
누락된 실행을 한 번 수행한다. 요청별 PDF 해시·자동 결과 해시·주택형·분양가를 대조한 후
...
대조·교정이 예약 시간에 자동 실행된다는 뜻도 아니다. 분석 실패 재시도는 기존 API 호출
