...
> 계약금은 분양가의 10%입니다. 중도금은 분양가의 60%입니다. 잔금은 분양가의 30%입니다. 공고문상 분양가의 40% 범위에서 중도금 대출을 알선할 예정입니다. 실제 실행과 개인 승인은 확정되지 않았습니다. 중도금 중 분양가의 20%는 사업장 알선 범위 밖이므로 별도 조달이 필요합니다. 취급은행은 공고문에 공개되지 않았습니다.

현재 JSON 계약은 additive v0.3을 유지하고 추출기 버전은 `0.2.3`입니다. 새 추출기는
`settlement_requirement`와 `risk_clauses[]`를 추가로 반환합니다. 검수본 조회는
PDF·공고·주택형·가격뿐 아니라 추출기 버전도 정확히 일치해야 하므로 이전 추출기
검수본이 현재 분석 결과로 재사용되지 않습니다.
...
`ADDITIONAL_COST_SCOPE_LIMITED`는 오류가 아닌 `blocking=false` 범위 안내입니다.
공고문에 별도 선택 유상옵션 안내가 있으며 `additional_costs`가 그 전체 카탈로그를
보장하지 않음을 뜻합니다. backend는 기본 자금판정을 계속할 수 있지만,
선택한 옵션 금액이 없는 상태를 “추가비용 전체 반영”으로 표시하면 안 됩니다.
