...
- Run `ruff check .`, `python -m pytest`, and `python -m compileall -q src tests` before reporting completion. Do not call the standalone `pytest` executable because this machine has multiple Python installations.
